...
Large language models (LLMs) often produce fluent, symbolically structured outputs that resemble multi-step reasoning, yet their behavior can appear \emph{incomplete}: intermediate steps fail to bind later conclusions, constraints are dropped, and outputs drift despite local coherence. We argue that this pattern is best explained by a distinct computational regime we call \emph{Partial Symbol Inductive Reasoning} (PSIR): systems with symbolic input--output interfaces whose internal dynamics are distributed and non-symbolic, but partially aligned with symbolic relations learned from language.

To make this diagnosis precise, we introduce \emph{Turing-rationality} as a criterion for when computation admits symbolic rational explanation. A computation is Turing-rational iff its transition from input to output can be abductively reconstructed as a sequence of symbolic intermediate states---\emph{reason-states}---that track inferential progress. We operationalize this with three tests for reason-states (type-level interpretability, alternative realizability, compositional evaluability) and a run-level requirement that reconstructed traces be run-specific, counterfactually discriminable, and narratively coherent.

Many physically grounded computations (including quantum and optical) compute Turing-computable functions yet fail Turing-rationality because their dynamics do not support stable symbolic intermediates. LLMs sit in between: PSIR systems exhibit partial compositional and inferential structure learned from language, but lack discrete symbolic commitments. As a result, they can reliably produce symbols while often lacking the internal reason-states needed for fully justifiable, “complete” reasoning.

We then define hallucination relative to a hypothesis language: a conclusion is hallucinatory when no consistent abductive constraint set exists that makes it necessary from the premises. Hallucination is thus not merely factual error but failure of symbolic justification---a predictable consequence of reasoning without stable commitments. Finally, we show how to scaffold PSIR systems externally: using counterexample-guided inductive synthesis (CEGIS), one can recover explicit symbolic theories from PSIR behavior and use them as rational guardrails for validation, stabilization, and repair without requiring the underlying model to become Turing-rational internally.
